...
def get_gradient_overlay(binary_mask, centrality_map, graph):
    ...
    node_coords = np.array([graph.nodes[n]['o'] for n in nodes]) # Assumes [y, x]
    ...
    # Find the nearest node for every pixel (brute force for small images, 

    # or use cKDTree for larger ones)
    ...
    # Map centrality to the grid

    pixel_centrality = centrality_vals[nearest_node_idx]

    

    # Normalize and apply coolwarm colormap
    ...
    colored_overlay = cmap(norm_centrality) # Returns (H, W, 4) in [0, 1]
